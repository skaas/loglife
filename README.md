...
3. 텍스트는 그대로 저장하고, 사진은 Google Drive 폴더에 업로드한 뒤 링크를 저장합니다.
...
- `GOOGLE_DRIVE_FOLDER_ID`: 사진을 넣을 Google Drive 폴더 id
- `GOOGLE_DRIVE_PUBLIC_LINKS`: `true` 면 업로드 직후 공개 읽기 권한을 붙여서 링크를 바로 열 수 있게 함
...
- 텍스트 메시지는 그대로 저장합니다.
- 링크 메시지는 일반 텍스트이므로 그대로 저장됩니다.
- 사진 메시지는 Google Drive 폴더에 업로드하고 링크만 노트에 남깁니다.
- 사진 업로드에는 Google Drive OAuth refresh token이 필요합니다.
- `GOOGLE_DRIVE_PUBLIC_LINKS=false` 이면 Drive 링크는 로그인된 본인 계정에서만 열립니다.
...
1. Google Cloud에서 Google Drive API를 활성화합니다.
2. OAuth client를 만들고 사용자 동의 화면에 본인 계정을 테스트 사용자로 추가합니다.
3. `https://www.googleapis.com/auth/drive.file` scope로 refresh token을 발급받습니다.
4. 업로드를 모아둘 Google Drive 폴더를 하나 만들고 URL에서 폴더 id를 복사합니다.
5. 아래 값을 Vercel 환경변수에 넣습니다.
...
```md
- 10:42 사진: 아침 커피
  [Google Drive에서 보기](https://drive.google.com/file/d/...)
```
